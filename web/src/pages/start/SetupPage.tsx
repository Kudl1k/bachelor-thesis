...
import { Battery, BatteryColumnType, fetchBatteryData } from "@/models/BatteryData";
import { Charger, ChargerColumnType, ChargerSearch,  searchChargerData, startTracking } from "@/models/ChargerData";
...
export function SetupPage() {
    ...
    const [selectedBatteries, setSelectedBatteries] = useState<Battery[] | null>(null);
    ...
    const [chargerData, setChargerData] = useState<ChargerColumnType[] | null>(null);
    ...
    useEffect(() => {
        if (batteries) {
            const transformedData = batteries.map(battery => ({
                ...
                last_charged_capacity: battery.last_charged_capacity ? battery.last_charged_capacity : "N/A",
                last_time_charged_at: battery.last_time_charged_at ? battery.last_time_charged_at : "N/A"
            }));
    ...
    useEffect(() => {
        ...
        setSelectedBatteries(batteries.filter(battery => selectedBatteryIds.includes(battery.id)));
    ...
    useEffect(() => {
        if (selectedChargerId > 0) {
            setSelectedCharger(chargers?.find(charger => charger.id === selectedChargerId) ?? null);
        }
    }, [chargers, selectedChargerId]);

    useEffect(() => {
        if (chargers) {
            setChargerData(chargers.map(charger => ({
                id: charger.id,
                name: charger.name,
                tty: charger.tty,
                slots: charger.slots,
                            ...
                            >
                                <CollapsibleTrigger className="w-full">
                                    <div className="flex w-full items-center justify-between pt-5">
                                        ...
                                        <div className="gap-1 flex items-center">
                                            <h4 className="font-bold">{selectedBatteries?.length}</h4>
                                ...
                                <CollapsibleContent>
                                    {selectedBatteries?.map((battery, index) => (
                                        <div key={battery.id} className="flex items-center justify-between">
                                            ...
                                            <h4 className="whitespace-nowrap">{battery.factory_capacity} mAh</h4>
                                        </div>
                                    ))}
                                </CollapsibleContent>
                            </Collapsible>
                    ...
                    <Card className="w-full">
                            ...
                            <>
                                <CardHeader>
                                    <CardTitle>
                                        <div className="flex w-full items-center justify-between">
                                            ...
                                            <div className="gap-1 flex items-center">
                                                <h4 className="font-bold">{selectedBatteryIds.length} selected</h4>
                                            </div>
                                        </div>
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="container mx-auto">
                                        <DataTable columns={batteryColumns} data={data} setSelectedIds={handleBatterySelectionChange} setSelectedId={() => {}} multiRowSelection/>
                                    </div>
                                    <div className="flex w-full justify-end pe-8">
                                        <Button disabled={isDisabledCharger} onClick={() => { handleContinueBattery() }}>
                                            Pick a charger
                                        </Button>
                                    </div>
                                </CardContent>
                        ...
                        {stage === Stage.CHARGER && (
                            <>
                                ...
                                <CardContent>
                                    <div className="container mx-auto">
                                        <DataTable columns={chargerColumns} data={chargerData || []} setSelectedId={handleChargerSelectionChange} setSelectedIds={() => {}} multiRowSelection={false} />
                                    </div>
                                    <div className="flex w-full justify-end ps-8 pe-8">
                                        
                                        <Button disabled={isFinish} onClick={() => { handleContinueCharger() }}>
                                            Go to summary
                                        </Button>
                                    </div>
                                </CardContent>
                ...
                <Card className="w-full xl:w-1/2 ms-4 me-4 mt-4">
                    ...
                    <CardContent>
                        <div className="grid grid-cols-2">
                            <div>
                                <h3 className="font-semibold text-xl pb-4">{selectedCharger?.name}</h3>
                                <p className="font-semibold">Baud rate: <span className="font-normal">{selectedCharger?.baudRate}</span></p>
                                <p className="font-semibold">Data bits: <span className="font-normal">{selectedCharger?.dataBits}</span></p>
                                <p className="font-semibold">Stop bits: <span className="font-normal">{selectedCharger?.stopBits}</span></p>
                                <p className="font-semibold">Parity: <span className="font-normal">{selectedCharger?.parity}</span></p>
                                <p className="font-semibold">RTS: <span className="font-normal">{selectedCharger?.rts && "Yes" || "No"}</span></p>
                                <p className="font-semibold">DTR: <span className="font-normal">{selectedCharger?.dtr && "Yes" || "No"}</span></p>
                                <p className="font-semibold">TTY: <span className="font-normal">{selectedCharger?.tty}</span></p>
                            </div>
                            <div>
